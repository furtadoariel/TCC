...
class Rules_Fuzzy(base.FuzzyReasonEngine):
    ...
    def reason(self):
        ...
        LOG.info('SAIDA FUZZY: %s', round(self.output.get("Output"), 0))
        # Verify if the situation its normal, it is then, verify the number of instances
        if round(self.output.get("Output"), 0) != 2.0 :
            self.notify_situation_to_controller(self.situations.get(round(self.output.get("Output"), 0)))
        else:
            if len(self.host_contexts_handler.get_active_instances().instances) == 0:
      	         self.notify_situation_to_controller(self.situations.get(4.0))
            else:
                self.notify_situation_to_controller(self.situations.get(round(self.output.get("Output"), 0)))
